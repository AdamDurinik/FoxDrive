...
using FoxHint.Admin.Data;   // <-- use AdminDbContext

namespace FoxHint.Admin
{
    public static class AdminTools
    {
        public static int Run(string[] args, IServiceProvider services)
        {
            if (args[0].Contains("help") || args.Length == 0)
            {
                ...
                Console.WriteLine("  dotnet run -- changepw <username> <newpassword>");
                return 1;
            }

            var db = services.GetRequiredService<AdminDbContext>();  // <-- here
            ...
            switch (args[0].ToLowerInvariant())
            {
                case "adduser":
                    ...
                    db.Users.Add(new User { Username = u, PasswordHash = hash });
                    db.SaveChanges();
